...
def suggest_hparams(trial: optuna.Trial, args: argparse.Namespace) -> dict[str, int | float | str | bool]:
    return suggest_hparams_from_search_space(
        ...
        required_keys=GNN_TRAIN_HPARAM_KEYS,
    )
...
def build_train_command(
    *,
    ...
    command = [
        ...
        "--gnn-dropout",
        str(hparams["gnn_dropout"]),
        "--head-dropout",
        str(hparams["head_dropout"]),
        "--edge-head-hidden-dim",
        str(hparams["edge_head_hidden_dim"]),
        "--hetero-aggr",
        str(hparams["hetero_aggr"]),
        "--no-verbose",
    ]
    command.append("--use-layer-norm" if bool(hparams["use_layer_norm"]) else "--no-use-layer-norm")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Optuna worker for GNN hyperparameter optimization. "
            "Run multiple copies of this script against the same study/storage for Slurm parallelism."
        )
    )
    parser.add_argument("--data-dir", type=Path, required=True, help="Gold GNN dataset directory.")
