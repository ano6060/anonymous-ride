...
from src.benchmark.utils.precision import SUPPORTED_PRECISIONS, normalize_precision
...
def suggest_hparams(
    ...
    return suggest_hparams_from_search_space(
        ...
        required_keys=MLP_TRAIN_HPARAM_KEYS,
    )
...
def build_train_command(
    *,
    ...
    output_dir: Path,
    seed: int,
    hparams: dict[str, str | int | float],
) -> list[str]:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Optuna worker for MLP hyperparameter optimization. "
